    ...
    public static void execute(LevelAccessor world, double x, double y, double z) {
        ...
        if (!UnderSecurity) {
            for (int index0 = 0; index0 < (int) getBlockNBTNumber(world, BlockPos.containing(x, y, z), "security-count"); index0++) {
                OutpostSecurity = world instanceof ServerLevel _level3 ? ModEntitiesInit.STANDART_SHOOTER_ENTITY.get().spawn(_level3, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED) : null;
        ...
        if (!HaveSquad && world.dayTime() == 1000) {
            cmd = world instanceof ServerLevel _level6 ? ModEntitiesInit.ELITE_SHOOTER_ENTITY.get().spawn(_level6, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED) : null;
            for (int index1 = 0; index1 < 4; index1++) {
                if (world instanceof ServerLevel _level) {
                    Entity entityToSpawn = ModEntitiesInit.STANDART_SHOOTER_ENTITY.get().spawn(_level, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED);
