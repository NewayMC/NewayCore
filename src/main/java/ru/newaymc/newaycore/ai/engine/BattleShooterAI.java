...
public class BattleShooterAI {
    public static void execute(LevelAccessor world, double x, double y, double z, Entity GetShooterEntity, double ammunation, double damage, double inaccurace_accumulation, double recoil, double recovery_time, double shoot, double speed,
                               String ai_type) {
        if (GetShooterEntity == null || ai_type == null)
            return;
        String aiState = "";
        double sx = 0;
        double sy = 0;
        ...
        if (!world.getEntitiesOfClass(Player.class, new AABB(Vec3.ZERO, Vec3.ZERO).move(new Vec3(x, y, z)).inflate(128 / 2d), e -> true).isEmpty()) {
            ...
            if (((Supplier<Boolean>) (() -> {
                ...
                if (((Supplier<Entity>) (() -> {
                    ...
                    if ((GetShooterEntity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) instanceof LivingEntity && (GetShooterEntity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null).isAlive()) {
                        ...
                        (GetShooterEntity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getOrCreateTag().putBoolean("IsMousePressed", true);
                        aiState = "inBattle";
                    } else {
                        (GetShooterEntity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getOrCreateTag().putBoolean("IsMousePressed", false);
                    }
                }
            } else {
                (GetShooterEntity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getOrCreateTag().putBoolean("IsMousePressed", false);
            ...
            if ((ai_type).equals("normal")) {
                ...
                if (!GetShooterEntity.getPersistentData().getBoolean("borderPatrolCall")) {
                    ...
                    if (GetShooterEntity instanceof PathfinderMob mob) {
                        mob.goalSelector.addGoal(3, new GoalsExtension.BorderPatrolGoal(mob, (double) 16, (double) 1, (int) 40));
                    }
                }
            }
            // Types setup
            ...
            if ((GetShooterEntity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getOrCreateTag().getBoolean("IsShooting")) {
                if (world instanceof Level _level) {
                    if (!_level.isClientSide()) {
                        _level.playSound(null, BlockPos.containing(GetShooterEntity.getX(), GetShooterEntity.getY(), GetShooterEntity.getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("newaycore:ak47_fire")), SoundSource.NEUTRAL, 1,
                                1);
                    } else {
            ...
            if (((Supplier<Boolean>) (() -> {
                ...
                if (world instanceof Level _level) {
                    if (!_level.isClientSide()) {
                        _level.playSound(null, BlockPos.containing(GetShooterEntity.getX(), GetShooterEntity.getY(), GetShooterEntity.getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("newaycore:ak47_reload")), SoundSource.NEUTRAL,
                                1, 1);
                    } else {
